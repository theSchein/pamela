...
import { withdrawRewardsL1Action } from '../../src/actions/withdrawRewardsL1';
...
describe('withdrawRewardsL1Action', () => {
  ...
  const MOCK_TX_HASH = '0xWithdrawRewardsTxHash';

  beforeEach(() => {
    resetCommonMocks();
    
    // Create standard test objects
    mockMessage = createMockMessage('Withdraw rewards from validator 123');
    mockState = createMockState([mockMessage]);
    mockCallback = createMockCallback();
    
    // Setup standard return values
    setupPolygonServiceMethod('withdrawRewards', MOCK_TX_HASH);
  ...
  describe('validate', () => {
    it('should return true if all required settings and services are available', async () => {
      ...
      const isValid = await withdrawRewardsL1Action.validate(mockRuntime, mockMessage, mockState);
  ...
  describe('handler', () => {
    it('should successfully withdraw rewards with valid validator ID from LLM', async () => {
      await withdrawRewardsL1Action.handler(
        mockRuntime, 
        mockMessage, 
        mockState, 
        undefined, 
      ...
      expect(mockPolygonRpcService.withdrawRewards).toHaveBeenCalledWith(VALID_VALIDATOR_ID);

      // Verify callback was called with success message
      expect(mockCallback).toHaveBeenCalledWith(
        expect.objectContaining({
          text: expect.stringContaining('Successfully initiated rewards withdrawal'),
    ...
    it('should handle errors from the service', async () => {
      const errorMessage = 'Withdrawal failed: no rewards available';
      setupPolygonServiceMethod('withdrawRewards', () => {
