...
// Import the previously mocked functions
...
describe('Polygon RPC L1/L2 Interactions', () => {
  ...
  beforeEach(() => {
    ...
    // Set up mock objects for getBlock and getTransaction
    mockL1PublicClient.getBlock = vi.fn().mockResolvedValue({ 
      hash: '0xL1BlockHash', 
      number: 16000000,
      timestamp: Math.floor(Date.now() / 1000)
    });
    
    mockL2PublicClient.getBlock = vi.fn().mockResolvedValue({ 
      hash: '0xL2BlockHash', 
      number: 40000000,
      timestamp: Math.floor(Date.now() / 1000)
    });
    
    mockL1PublicClient.getTransaction = vi.fn().mockResolvedValue({ 
      hash: '0xL1TxHash', 
      from: '0xSenderAddress', 
      to: '0xRecipientAddress' 
    });
    
    mockL2PublicClient.getTransaction = vi.fn().mockResolvedValue({ 
      hash: '0xL2TxHash', 
      from: '0xSenderAddress', 
      to: '0xRecipientAddress' 
    });
    
    // Set up mock returns for sendTransaction
    mockL1WalletClient.sendTransaction = vi.fn().mockResolvedValue('0xL1SendTxHash');
    mockL2WalletClient.sendTransaction = vi.fn().mockResolvedValue('0xL2SendTxHash');
  ...
  describe('Network-Specific Operations', () => {
    ...
    it('should retrieve token balances from the correct network', async () => {
      // Mock readContract for ERC20 balanceOf calls
      mockL1PublicClient.readContract = vi.fn().mockResolvedValue(BigInt(1000000000000000000));
      mockL2PublicClient.readContract = vi.fn().mockResolvedValue(BigInt(2000000000000000000));
    ...
    it('should retrieve different token metadata from each network', async () => {
      // Mock readContract for ERC20 metadata calls
      mockL1PublicClient.readContract = vi.fn().mockImplementation(({ functionName }) => {
        if (functionName === 'symbol') return 'ETH-TOKEN';
        if (functionName === 'decimals') return 18;
        if (functionName === 'name') return 'ETH Test Token';
        return null;
      });

      mockL2PublicClient.readContract = vi.fn().mockImplementation(({ functionName }) => {
        if (functionName === 'symbol') return 'MATIC-TOKEN';
        if (functionName === 'decimals') return 18;
        if (functionName === 'name') return 'Polygon Test Token';
        return null;
      });
  ...
  describe('Transaction Sending', () => {
    ...
    it('should execute contract interactions on the correct network', async () => {
      ...
      // Check that the calls included the token address
  ...
  describe('Cross-Chain Operations', () => {
    ...
    it('should default to L2 when no chain is specified', async () => {
      ...
      mockL2PublicClient.getBlockNumber.mockResolvedValue(40000000);
      mockL2PublicClient.readContract.mockResolvedValue(BigInt(100000000000000000000));
