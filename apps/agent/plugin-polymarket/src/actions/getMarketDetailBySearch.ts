...
export const getMarketDetailBySearchAction: Action = {
  ...
  handler: async (
    ...
    try {
      ...
      if (!searchQuery) {
        const errorMsg =
          'Please provide a search term. For example: "Find markets about AI" or "Show me election markets"';
        if (callback) {
          callback({
            text: errorMsg,
            success: false,
      ...
      if (markets.length === 0) {
        elizaLogger.warn(
          `No database results for "${searchQuery}", trying fallback to popular markets`,
        );
        try {
          markets = await marketDetailService.getPopularMarkets(undefined, 3);
          if (markets.length > 0) {
            elizaLogger.info(
              `Fallback successful: showing ${markets.length} popular markets instead`,
            );
          }
        } catch (fallbackError) {
          elizaLogger.error(
            "Fallback to popular markets also failed:",
            fallbackError,
          );
        }
      }
