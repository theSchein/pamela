...
describe('Provider Tests', () => {
  // Find the POLYMARKET_PROVIDER from the providers array
  const polymarketProvider = plugin.providers?.find(
    (provider) => provider.name === 'POLYMARKET_PROVIDER'
  );

  describe('POLYMARKET_PROVIDER', () => {
    it('should exist in the plugin', () => {
      ...
      if (plugin.providers) {
        ...
        const result = plugin.providers.find((p) => p.name === 'POLYMARKET_PROVIDER');
    ...
    it('should have the correct structure', () => {
      if (polymarketProvider) {
        expect(polymarketProvider).toHaveProperty('name', 'POLYMARKET_PROVIDER');
        expect(polymarketProvider).toHaveProperty('description');
        expect(polymarketProvider).toHaveProperty('get');
        expect(typeof polymarketProvider.get).toBe('function');

        documentTestResult('Provider structure check', {
          name: polymarketProvider.name,
          description: polymarketProvider.description,
          hasGetMethod: typeof polymarketProvider.get === 'function',
        });
      }
    });

    it('should have a description explaining its purpose', () => {
      if (polymarketProvider && polymarketProvider.description) {
        expect(typeof polymarketProvider.description).toBe('string');
        expect(polymarketProvider.description.length).toBeGreaterThan(0);
    ...
    it('should return provider data from the get method', async () => {
      if (polymarketProvider) {
        ...
        try {
          ...
          result = await polymarketProvider.get(runtime, message, state);
    ...
    it('should handle error conditions gracefully', async () => {
      if (polymarketProvider) {
        ...
        try {
          ...
          result = await polymarketProvider.get(runtime, invalidMemory, state);
