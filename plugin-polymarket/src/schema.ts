...
export const polymarketMarketsTable = pgTable(
  ...
  (table) => ({
    ...
    conditionIdIdx: index('polymarket_markets_condition_id_idx').on(table.conditionId),
    ...
    lastSyncedIdx: index('polymarket_markets_last_synced_idx').on(table.lastSyncedAt),
    
    // Active markets are most commonly queried
    activeClosedIdx: index('polymarket_markets_active_closed_idx').on(table.active, table.closed),
  })
);
...
export const polymarketTokensTable = pgTable(
  ...
  (table) => ({
    ...
    conditionIdIdx: index('polymarket_tokens_condition_id_idx').on(table.conditionId),
    outcomeIdx: index('polymarket_tokens_outcome_idx').on(table.outcome),
    
    // Compound index for market + outcome lookups
    conditionOutcomeIdx: index('polymarket_tokens_condition_outcome_idx').on(table.conditionId, table.outcome),
  })
);
...
export const polymarketRewardsTable = pgTable(
  'polymarket_rewards',
  {
    ...
    inGameMultiplier: decimal('in_game_multiplier', { precision: 10, scale: 4 }),
  ...
  (table) => ({
    ...
    conditionIdUnique: unique('polymarket_rewards_condition_id_unique').on(table.conditionId),
...
export const polymarketPricesTable = pgTable(
  ...
  (table) => ({
    ...
    conditionIdIdx: index('polymarket_prices_condition_id_idx').on(table.conditionId),
    updatedAtIdx: index('polymarket_prices_updated_at_idx').on(table.updatedAt),
    
    // Most recent price per token
    tokenUpdatedIdx: index('polymarket_prices_token_updated_idx').on(table.tokenId, table.updatedAt),
  })
);
...
export const polymarketSyncStatusTable = pgTable(
  ...
  (table) => ({
    ...
    lastSyncIdx: index('polymarket_sync_status_last_sync_idx').on(table.lastSyncAt),
    statusIdx: index('polymarket_sync_status_status_idx').on(table.syncStatus),
    
    // Latest sync per type
    typeLastSyncIdx: index('polymarket_sync_status_type_last_sync_idx').on(table.syncType, table.lastSyncAt),
  })
);
...
export type PolymarketSyncStatus = typeof polymarketSyncStatusTable.$inferSelect;
export type NewPolymarketSyncStatus = typeof polymarketSyncStatusTable.$inferInsert;