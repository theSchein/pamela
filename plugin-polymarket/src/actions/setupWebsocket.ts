...
export const setupWebsocketAction: Action = {
  ...
  validate: async (
    ...
    const clobWsUrl =
      ...
      "wss://ws-subscriptions-clob.polymarket.com/ws"; // Note: /ws path segment might be added later
    if (!clobWsUrl) {
      logger.warn(
    ...
    logger.info(
      "[setupWebsocketAction] CLOB_WS_URL found. Further validation in handler.",
    );
    return true;
  },
  ...
  handler: async (
    ...
    logger.info(
      "[setupWebsocketAction] Handler called - now using ws library.",
    );
    ...
    if (existingClient && typeof existingClient.terminate === "function") {
      ...
      try {
        ...
        logger.warn(
          "[setupWebsocketAction] Error terminating existing client, proceeding.",
          e,
        );
      }
    }
    ...
    try {
      logger.info(
        "[setupWebsocketAction] Attempting LLM parameter extraction...",
      );
      ...
      logger.info(
        `[setupWebsocketAction] LLM raw parameters: ${JSON.stringify(extractedParams)}`,
      );
      ...
      if (
        ...
        return createErrorResult(
          "LLM failed to find parameters, trying regex.",
        );
      }
      ...
      if (extractedMarkets || extractedUserId) {
        ...
        return createErrorResult(
          "LLM did not return markets or userId, trying regex.",
        );
      }
    } catch (error: any) {
      logger.warn(
      ...
      const userRegex =
        /user(?:Id)?(?:\\s*[:\\-]?\\s*)((?:0x)?[0-9a-fA-F]{40})/i;
      const userMatch = text.match(userRegex);
      if (userMatch && userMatch[1]) {
        extractedUserId = userMatch[1];
        logger.info(
          `[setupWebsocketAction] Regex fallback extracted userId: ${extractedUserId}`,
        );
      }
    }

    if (
      (!extractedMarkets || extractedMarkets.length === 0) &&
      !extractedUserId
    ) {
      const errorMsg =
    ...
    const baseWsUrl =
      runtime.getSetting("CLOB_WS_URL") ||
      "wss://ws-subscriptions-clob.polymarket.com/ws"; // Ensure /ws is part of base or added
    const subscriptionType: "user" | "market" = extractedUserId
      ? "user"
      : "market"; // Prefer user if userId is present
    ...
    if (subscriptionType === "user") {
      if (!extractedUserId) {
        ...
        const errMsg =
          "User ID is required for user channel subscription but was not found.";
      ...
      if (!apiKey || !apiSecret || !apiPassphrase) {
        ...
        logger.error(
          `[setupWebsocketAction] Missing credentials for user subscription: ${errMsg}`,
        );
        if (callback) await callback({ text: `❌ ${errMsg}` });
        return createErrorResult(errMsg);
      }
    ...
    return new Promise<ActionResult>((resolvePromise, rejectPromise) => {
      try {
        logger.info(
          `[setupWebsocketAction] Creating new WebSocket connection to: ${wsUrl}`,
        );
        ...
        wsClient.on("open", () => {
          ...
          wsClient.send(messageStr, (err?: Error) => {
            if (err) {
              ...
              rejectPromise(
                new Error(`Failed to send subscription: ${err.message}`),
              );
              return;
            }
            logger.info(
              `[setupWebsocketAction] Subscription message sent: ${messageStr}`,
            );
        ...
        wsClient.on("error", (error: Error) => {
          logger.error(
            "[setupWebsocketAction] WebSocket connection error:",
            error,
          );
          setActiveClobSocketClientReference(null);
    ...
    [
      {
        name: "{{user1}}",
        content: {
          text: "Connect to Polymarket WebSocket and subscribe to market 0xMarket123.",
        },
      },
      {
        name: "{{user2}}",
    ...
    [
      {
        name: "{{user1}}",
        content: {
          text: "Start a realtime feed for markets 0xMarket1, 0xMarket2.",
        },
      },
      {
        name: "{{user2}}",
