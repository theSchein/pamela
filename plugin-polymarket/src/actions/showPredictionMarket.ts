...
export const showPredictionMarketAction: Action = {
  ...
  validate: async (_runtime: IAgentRuntime, message: Memory) => {
    ...
    const isDirect = directMarketKeywords.some(keyword => text.includes(keyword));
    if (isDirect) return true;

    // Secondary match - general market requests without search intent
    const hasMarketWord = text.includes('market');
    const hasShowWord = text.includes('show') || text.includes('get') || text.includes('display');
    const noSearchIntent = !text.includes('search') && !text.includes('find') && !text.includes('about');
  ...
  handler: async (
    ...
    elizaLogger.info('[showPredictionMarketAction] Handler called - using database-first approach');
