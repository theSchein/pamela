...
export const getPriceHistory: Action = {
  ...
  handler: async (
    ...
    try {
      ...
      if (!params.tokenId) {
        return createErrorResult(
          "Token ID is required for price history retrieval",
        );
      }
    ...
    [
      {
        name: "{{user1}}",
        content: {
          text: "Get price history for token 123456 with 1d interval via Polymarket",
        },
      },
      {
        name: "{{user2}}",
    ...
    [
      {
        name: "{{user1}}",
        content: {
          text: "Show me 1h price chart for token 456789 via Polymarket",
        },
      },
      {
        name: "{{user2}}",
...
function formatPriceHistoryResponse(
  ...
  if (sortedHistory.length >= 2) {
    ...
    const earliestDate = new Date(
      sortedHistory[sortedHistory.length - 1].t * 1000,
    );
