  ...
  handler: async (
    ...
    try {
      ...
      try {
        ...
        return createErrorResult('Failed to extract token ID from message. Please specify a token ID.');
      }

      // Validate required parameters
      if (!params.tokenId) {
        return createErrorResult('Token ID is required for price history retrieval');
      }
