  ...
  handler: async (
    ...
    try {
      ...
      try {
        ...
        trades = Array.isArray(tradesResponse) ? tradesResponse : (tradesResponse?.data || []);
        logger.info(`[getPortfolioPositions] Retrieved ${trades.length} trades`);
      } catch (tradesError) {
        logger.warn('[getPortfolioPositions] Failed to get trades:', tradesError);
      }
      ...
      try {
        ...
        openOrders = Array.isArray(ordersResponse) ? ordersResponse : (ordersResponse?.data || []);
        logger.info(`[getPortfolioPositions] Retrieved ${openOrders.length} open orders`);
      } catch (ordersError) {
        logger.warn('[getPortfolioPositions] Failed to get open orders:', ordersError);
      }
      ...
      positions = Array.from(positionMap.values()).filter(pos => Math.abs(pos.totalSize) > 0.001);
      ...
      for (const position of positions) {
        try {
          const avgPrice = position.totalSize > 0 ? position.totalValue / position.totalSize : 0;
      ...
      try {
        ...
        usdcBalance = (parseFloat(balanceResponse.balance || '0') / 1000000).toFixed(6);
      } catch (balanceError) {
        logger.warn('[getPortfolioPositions] Failed to get balance:', balanceError);
      }
...
${openOrders.length > 0 ? `**Open Orders**: ${openOrders.length} pending
    ...
    '**Status**: No positions detected. Place some orders to see holdings here.'
        ...
        const positionsDisplay = processedPositions
          .map((pos, index) => {
            ...
            const avgPrice = pos.averagePrice ? parseFloat(pos.averagePrice) : value / size;
