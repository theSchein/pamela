...
export const getEnhancedMarketInfoAction: Action = {
  ...
  description: 'Get detailed information about a specific market by condition_id',
  examples: [
    [
      {
        name: 'Human',
  ...
  validate: async (_runtime: IAgentRuntime, message: Memory) => {
    ...
    const hasMarketKeywords = [
      'market details', 'market info', 'tell me about', 'what is',
      'market information', 'detailed info'
  ...
  handler: async (
    ...
    try {
      ...
      if (!conditionIdMatch) {
        const errorMsg = 'Please provide a valid condition_id (32+ character hex string). Example: "Tell me about market 0x1234567890abcdef..."';
        if (callback) {
          callback({
            text: errorMsg,
            success: false,
      ...
      elizaLogger.info(`Getting enhanced market info for condition_id: ${conditionId}`);
      
      // Get enhanced market information
      const marketInfo = await marketDetailService.getEnhancedMarketInfo(conditionId);
      ...
      const question = liveMarket?.question || localMarket?.question || 'Unknown';
      const marketSlug = liveMarket?.market_slug || localMarket?.marketSlug || 'Unknown';
      const category = liveMarket?.category || localMarket?.category || 'Uncategorized';
      const active = liveMarket?.active !== undefined ? liveMarket.active : localMarket?.active;
      const closed = liveMarket?.closed !== undefined ? liveMarket.closed : false;
      ...
      const gameStartTime = liveMarket?.game_start_time || localMarket?.gameStartTime;
      if (gameStartTime) {
      ...
      // Trading parameters
      const minOrderSize = liveMarket?.minimum_order_size || localMarket?.minimumOrderSize;
      if (minOrderSize) {
        response += `💰 **Min Order Size:** ${minOrderSize}\n`;
      }
      
      const minTickSize = liveMarket?.minimum_tick_size || localMarket?.minimumTickSize;
      if (minTickSize) {
