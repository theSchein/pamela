...
import type {
  OpenOrder,
  GetOpenOrdersParams as LocalGetOpenOrdersParams,
} from "../types";
import {
...
export const getActiveOrdersAction: Action = {
  ...
  validate: async (
    ...
    logger.info(
      `[getActiveOrdersAction] Validate called for message: "${message.content?.text}"`,
    );
  ...
  handler: async (
    ...
    try {
      ...
      logger.info(
        `[getActiveOrdersAction] LLM result: ${JSON.stringify(extractedParams)}`,
      );

      if (extractedParams.error || !extractedParams.marketId) {
        return createErrorResult(
          extractedParams.error || "Market ID not found in LLM result.",
        );
      }
    } catch (error) {
      logger.warn(
        "[getActiveOrdersAction] LLM extraction failed, trying regex fallback",
        error,
      );
      const text = message.content?.text || "";
      const marketRegex =
        /(?:market|marketId|condition_id)[:\s]?([0-9a-zA-Z_.-]+)/i;
      ...
      if (!extractedParams.marketId) {
        ...
        logger.error(
          `[getActiveOrdersAction] Market ID extraction failed. Text: "${text}"`,
        );
    ...
    try {
      ...
      logger.error(
        "[getActiveOrdersAction] Error fetching active orders:",
        error,
      );
      const errorMessage =
