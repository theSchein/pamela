...
Checking current approvals...`,
      ...
      const bridgedBalance = await usdcBridgedContract.balanceOf(wallet.address);
...
${currentStatus.usdcForCTF && currentStatus.usdcForExchange && currentStatus.ctfForExchange && currentStatus.usdcForNegRisk && currentStatus.ctfForNegRisk
  ...
  : 'Setting missing approvals...'
          ...
          actions: ['APPROVE_USDC'],
          data: { currentStatus, usdcType, balance: ethers.formatUnits(useNativeUSDC ? nativeBalance : bridgedBalance, 6) },
        };
...
**Contract**: ${CTF_ADDRESS}`,
        ...
        const tx1 = await usdcContract.approve(CTF_ADDRESS, ethers.MaxUint256, { gasLimit });
...
**Contract**: ${EXCHANGE_ADDRESS}`,
        ...
        const tx2 = await usdcContract.approve(EXCHANGE_ADDRESS, ethers.MaxUint256, { gasLimit });
        const receipt2 = await tx2.wait();
        transactions.push({ step: 2, contract: 'Exchange', txHash: receipt2.hash });
...
**Contract**: ${EXCHANGE_ADDRESS}`,
        ...
        const tx3 = await ctfContract.setApprovalForAll(EXCHANGE_ADDRESS, true, { gasLimit });
        const receipt3 = await tx3.wait();
        transactions.push({ step: 3, contract: 'CTF for Exchange', txHash: receipt3.hash });
