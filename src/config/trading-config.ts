...
export function validateTradingConfig(config: TradingConfig): boolean {
    ...
    if (config.riskLimitPerTrade <= 0 || config.riskLimitPerTrade > config.maxPositionSize) {
        throw new Error('riskLimitPerTrade must be positive and less than maxPositionSize');
    }
    return true;
}