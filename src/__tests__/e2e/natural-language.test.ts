  ...
  tests = [
    {
      ...
      fn: async (runtime: any) => {
        ...
        try {
          ...
          throw new Error(
            `Hello world test failed: ${(error as Error).message}`,
          );
        }
      },
    },
      ...
      fn: async (runtime: any) => {
        ...
        try {
          ...
          for (const greeting of greetings) {
            ...
            if (!agentResponse) {
              throw new Error(
                `Agent did not respond to greeting: "${greeting}"`,
              );
            }
            ...
            if (
              ...
              agentResponse.content.text.length < 2
            ) {
          ...
          throw new Error(
            `Casual greeting test failed: ${(error as Error).message}`,
          );
        }
      },
    },
      ...
      fn: async (runtime: any) => {
        ...
        try {
          ...
          if (messages.length < 4) {
            throw new Error(
              `Expected at least 4 messages, got ${messages.length}`,
            );
          }

          // Find the agent's second response
          const agentResponses = messages.filter(
            (m: any) => m.userId === runtime.agentId,
          );
