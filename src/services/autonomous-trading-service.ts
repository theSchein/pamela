import { elizaLogger, IAgentRuntime, Service, Memory, UUID, Content, HandlerCallback, State } from "@elizaos/core";
import { DEFAULT_TRADING_CONFIG, TradingConfig } from "../config/trading-config.js";
import { 
    getMarketsToMonitor, 
...
export class AutonomousTradingService extends Service {
    ...
    static async start(runtime: IAgentRuntime): Promise<AutonomousTradingService> {
    ...
    async initialize(runtime: IAgentRuntime): Promise<void> {
        ...
        elizaLogger.info("Configuration: " + JSON.stringify({
        ...
        if (!this.tradingConfig.unsupervisedMode) {
            elizaLogger.info("⚠️  UNSUPERVISED MODE DISABLED - Running in monitoring mode only");
            elizaLogger.info("📊 Markets will be scanned but NO trades will be executed");
            elizaLogger.info("💡 To enable trading: Set UNSUPERVISED_MODE=true in .env");
        } else {
            elizaLogger.info("🚨 UNSUPERVISED MODE ENABLED - Trades will be executed automatically!");
        }
    ...
    private async loadExistingPositions(): Promise<void> {
        try {
            ...
            elizaLogger.info(`Loaded ${this.openPositions.size} open positions from wallet`);
        } catch (error) {
    ...
    private startAutonomousTrading(): void {
        ...
        this.isRunning = true;
        elizaLogger.info("📡 Starting market monitoring - scanning every 60 seconds");
        elizaLogger.info(`🎯 Monitoring ${getMarketsToMonitor()?.length || 0} hardcoded markets`);
        ...
        try {
            ...
            if (opportunities.length > 0) {
                ...
                for (const opp of opportunities) {
                    elizaLogger.info(`  📈 ${opp.question}: ${opp.outcome} @ ${(opp.currentPrice * 100).toFixed(1)}%`);
                }
            } else {
            ...
            for (const opportunity of opportunities) {
                ...
                if (decision.shouldTrade) {
                    if (this.tradingConfig.unsupervisedMode) {
                        ...
                        elizaLogger.info(`📋 WOULD TRADE (monitoring mode): ${opportunity.question}`);
    ...
    private async findSimpleStrategyOpportunities(): Promise<MarketOpportunity[]> {
        ...
        try {
            ...
            elizaLogger.info(`Checking ${marketIds.length} hardcoded markets for simple strategy opportunities`);

            for (const conditionId of marketIds) {
                // Skip if we already have a position in this market
                if (this.openPositions.has(conditionId)) {
                    elizaLogger.debug(`Skipping ${conditionId.slice(0, 10)}... - already have position`);
                    continue;
                }
                ...
                if (!market || !market.active) {
                    elizaLogger.debug(`Market ${conditionId.slice(0, 10)}... not found or inactive`);
                    continue;
                }
                ...
                for (let i = 0; i < outcomes.length; i++) {
                    ...
                    if (price <= config.BUY_THRESHOLD) {
                        ...
                        if (edge >= config.MIN_EDGE) {
                            elizaLogger.info(`Simple strategy opportunity found: ${market.question}`);
                            elizaLogger.info(`  ${outcomeName} at ${(price * 100).toFixed(1)}% (below ${config.BUY_THRESHOLD * 100}% threshold)`);
                            ...
                            opportunities.push({
                                ...
                                newsSignals: [`Simple strategy: ${outcomeName} below ${config.BUY_THRESHOLD * 100}%`],
                                riskScore: 0.1
                            });
                        }
                    }
                    ...
                    if (outcomeName === 'YES' && price >= config.SELL_THRESHOLD) {
                        ...
                        if (edge >= config.MIN_EDGE && noPrice <= config.BUY_THRESHOLD) {
                            elizaLogger.info(`Simple strategy opportunity found (inverse): ${market.question}`);
                            elizaLogger.info(`  NO at ${(noPrice * 100).toFixed(1)}% (YES above ${config.SELL_THRESHOLD * 100}% threshold)`);
                            ...
                            opportunities.push({
                                ...
                                confidence: 0.9,
                                expectedValue: edge * 100,
                                newsSignals: [`Simple strategy: YES above ${config.SELL_THRESHOLD * 100}%, buying NO`],
                                riskScore: 0.1
                            });
                        }
                    }
            ...
            elizaLogger.info(`Found ${opportunities.length} simple strategy opportunities`);
    ...
    private async analyzeMarket(market: any): Promise<MarketOpportunity | null> {
        try {
            ...
            const predictedProbability = await this.calculateProbability(market, newsSignals);
            
            // Determine best outcome to trade
            const yesPrice = market.outcomes?.find((o: any) => o.name === 'Yes')?.price || 0.5;
            const noPrice = market.outcomes?.find((o: any) => o.name === 'No')?.price || 0.5;
    ...
    private async calculateProbability(market: any, _newsSignals: string[]): Promise<number> {
        try {
    ...
    private calculateRiskScore(market: any, edgeSize: number): number {
        ...
        const timeRisk = market.endDate && 
            new Date(market.endDate).getTime() - Date.now() < 86400000 ? 0.3 : 0;
        const edgeRisk = edgeSize < 0.1 ? 0.4 : 0;
        
        return volumeRisk + timeRisk + edgeRisk;
    }

    private async evaluateOpportunity(opportunity: MarketOpportunity): Promise<TradingDecision> {
    ...
    private calculatePositionSize(opportunity: MarketOpportunity): number {
        ...
        const edge = Math.abs(opportunity.predictedProbability - opportunity.currentPrice);
        ...
        const rawSize = kellyFraction * conservativeFactor * this.tradingConfig.maxPositionSize;
        
        // Apply risk limit
        const riskAdjustedSize = Math.min(rawSize, this.tradingConfig.riskLimitPerTrade);
        
        // Round to nearest dollar
        return Math.floor(riskAdjustedSize);
    }

    private generateTradingReasoning(opportunity: MarketOpportunity, shouldTrade: boolean): string {
        const reasons = [];
        
        if (shouldTrade) {
            reasons.push(`High confidence trade (${(opportunity.confidence * 100).toFixed(1)}%)`);
            reasons.push(`Expected value: $${opportunity.expectedValue.toFixed(2)}`);
            
            if (opportunity.newsSignals.length > 0) {
                reasons.push(`Supported by ${opportunity.newsSignals.length} news signals`);
            }
            
            reasons.push(`Predicted probability: ${(opportunity.predictedProbability * 100).toFixed(1)}%`);
            reasons.push(`Current price: ${(opportunity.currentPrice * 100).toFixed(1)}%`);
        } else {
            if (opportunity.confidence < this.tradingConfig.minConfidenceThreshold) {
                reasons.push(`Confidence too low (${(opportunity.confidence * 100).toFixed(1)}%)`);
            }
            if (opportunity.expectedValue <= 5) {
                reasons.push(`Expected value too small ($${opportunity.expectedValue.toFixed(2)})`);
            }
            if (opportunity.riskScore > 0.5) {
                reasons.push(`Risk score too high (${(opportunity.riskScore * 100).toFixed(1)}%)`);
            }
        }
    ...
    private async executeTrade(decision: TradingDecision): Promise<void> {
        try {
            elizaLogger.info("Executing autonomous trade: " + JSON.stringify({
            ...
            if (tradeResult.success) {
                ...
                elizaLogger.info("Trade executed successfully: " + tradeResult.orderId + " for market " + decision.marketId);
            } else {
                elizaLogger.error("Trade execution failed for market " + decision.marketId + ": " + tradeResult.error);
            }
        } catch (error) {
    ...
    private async placeOrder(decision: TradingDecision): Promise<any> {
        try {
            ...
            const tokenId = decision.outcome === 'YES' ? clobTokenIds[0] : clobTokenIds[1];
            
            if (!tokenId) {
                elizaLogger.error("Could not find token ID for outcome " + decision.outcome);
            ...
            if (orderValueUSDC < 1) {
                elizaLogger.warn(`Order value $${orderValueUSDC} is below minimum $1, adjusting to minimum`);
            ...
            elizaLogger.info(`Placing direct order: ${decision.size} USDC (${sharesAmount.toFixed(2)} shares) of ${decision.outcome} @ ${decision.price} for "${market.question}"`);
