import { elizaLogger, IAgentRuntime, UUID, Content, HandlerCallback } from "@elizaos/core";
...
export async function placeDirectOrder(
    ...
    try {
        ...
        elizaLogger.info(`[placeDirectOrder] Parameters: ${JSON.stringify(params)}`);
        ...
        elizaLogger.info(`[placeDirectOrder] Placing ${orderType} order: ${params.side} ${params.size} @ ${params.price} for token ${params.tokenId}`);
        ...
        if (resp && resp.orderID) {
            elizaLogger.info(`[placeDirectOrder] Order placed successfully: ${resp.orderID}`);
